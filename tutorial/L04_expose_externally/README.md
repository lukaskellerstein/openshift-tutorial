# LP-L04 — Expose Services Externally: Routes and TLS
...
- Completed: [L01 — Projects](../L01_projects/), [L02 — Build & Image Resources](../L02_builds_and_images/), [L03 — Deploy the Microservices Stack](../L03_deploy_microservices/)
...
- Traefik middleware (rate limiting, circuit breakers) — use Route annotations or the Service Mesh (L05) instead
...
Your services are now accessible from outside the cluster. Run this to print the URLs:
...
Open the **Dashboard UI** URL in your browser — you should see the ShopInsights React dashboard with products, orders, and analytics data.

The three API services use FastAPI, which includes **Swagger UI** out of the box at `/docs`. Open any of the Swagger URLs to browse and test every endpoint interactively — no curl needed.

> **"Application is not available"?** The Developer Sandbox idles workloads after inactivity (scales deployments to 0 replicas). If you see this error, scale them back up:
> ```bash
> oc scale deployment products-service orders-service analytics-service dashboard-ui --replicas=1
> ```
...
### Step 8: View Routes in the Web Console
...
The Web Console also shows Route metrics (request rate, error rate, latency) if monitoring is enabled — you will explore this in L07.
...
```bash
oc delete route -l tutorial=personalized,lesson=04
```
...
Note: This only removes the Routes. The underlying Services and Deployments from L03 remain running. The services are just no longer accessible from outside the cluster.

## Next Steps

Your services are now accessible from outside the cluster, but all traffic between services inside the cluster is unencrypted and unobserved. In [L05: Service Mesh with Istio](../L05_service_mesh/), you will add automatic mTLS, traffic management (canary deployments), and observability (Kiali service graph).
