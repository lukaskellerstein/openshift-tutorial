# L1-M2.2 — Recipes Catalog: Pre-Built AI Applications
...
3. Select a **model** for the recipe. Choose the Granite model you downloaded in L1-M2.1, or let the recipe download a recommended model.
4. Click **Start**.
...
The answers should be grounded in the document you uploaded. Compare this to asking the same question in the L1-M2.1 playground (without RAG) -- the playground model may give a generic answer from training data, while the RAG chatbot cites your specific document content.
...
Notice the pattern: the application code communicates with the model through the OpenAI-compatible API. The application does not load the model directly -- it makes HTTP calls to the inference server, just like you did with `curl` in L1-M2.1.
...
In [L1-M2.3 — From Podman AI Lab to Production](../3_to_production/), you will learn how to take what you have built locally -- model choices, application code, and configuration -- and map it to a production deployment on RHEL AI or OpenShift AI.
