...
- Completed: [L1-2.2 — Recipes Catalog](../2_recipes_catalog/)
...
In L1-2.1, you chose a model from the catalog (e.g., `granite-3.1-8b-instruct`). Here is how that choice maps to each production tier:
...
The RAG Chatbot recipe you ran in L1-2.2 is a containerized application. To deploy it in production:
...
The playground settings you tuned in L1-2.1 (temperature, max tokens, system prompt) become production configuration:
...
- **RHEL AI documentation** at [docs.redhat.com](https://docs.redhat.com/en/documentation/red_hat_enterprise_linux_ai/3.4) -- the next tier in the deployment model
...
In [L1-3.1 — RHEL AI Architecture and Concepts](../../M3_rhel_ai/1_architecture_and_concepts/), you will explore the next tier: RHEL AI -- a bootable container image that turns a bare-metal server into a purpose-built AI inference and model adaptation platform, powered by Red Hat AI Inference Server (vLLM-based) and the ilab CLI.
