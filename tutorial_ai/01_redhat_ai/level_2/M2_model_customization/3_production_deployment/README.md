...
- Completed L2-2.1 (Modular Model Customization) and L2-2.2 (Model Optimization) -- you have a fine-tuned or optimized model ready to serve
...
RHEL AI uses the Red Hat AI Inference Server (vLLM-based) as its inference engine. It provides an OpenAI-compatible API server that supports all of these requirements through configuration flags and environment variables.

> **Note:** RHEL AI 3.4 ships with a built-in Quadlet service at `/etc/containers/systemd/rhaii.container` (renamed from `rhaiis` in earlier versions). For production customization, we create our own systemd unit files below to have full control over the configuration.
...
Continue to [L2-3.1 -- End-to-End: Podman AI Lab to RHEL AI to OpenShift AI](../../M3_cross_tier_workflows/1_end_to_end/) to see how a model moves through all three tiers of the Red Hat AI stack, from desktop prototyping through fine-tuning to production deployment at scale.
